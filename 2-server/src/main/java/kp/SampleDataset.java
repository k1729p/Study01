...
public class SampleDataset {
	...
	private static Map<Long, Department> departmentMap = new TreeMap<>();
	 ...
	 */
	public static void loadDataset(Optional<Long> depIndex, Optional<Long> empIndex) {

		depIndexUpperBound = depIndex.orElse(DEP_INDEX_UPPER_BOUND);
		empIndexUpperBound = empIndex.orElse(EMP_INDEX_UPPER_BOUND);
		departmentMap.clear();
		generateDepartments();
	}
	...
	public static Optional<Department> mergeDepartment(Long id, Department department) {
		...
		return Optional.ofNullable(departmentMap.merge(id, department, mapper));
	}
	...
	public static Optional<Employee> removeEmployee(Long departmentId, Long employeeId) {
		...
		final Optional<Employee> existingEmployeeOpt = getEmployee(departmentId, employeeId);
		if (existingEmployeeOpt.isPresent()) {
			departmentMap.get(departmentId).getEmployees().remove(existingEmployeeOpt.get());
		}
		return existingEmployeeOpt;
	}

	/**
	 * Gets the list of {@link Employee}s from the sample dataset.
	 * 
	 * @param departmentId the {@link Department}'s id
	...
	public static Optional<Employee> getEmployee(Long departmentId, Long employeeId) {
		...
		if (Objects.isNull(departmentId) || Objects.isNull(employeeId)
				|| Objects.isNull(departmentMap.get(departmentId))) {
			return Optional.empty();
		}
		final Department department = departmentMap.get(departmentId);
		return Optional.ofNullable(department.getEmployees())
				.map(list -> list.stream().filter(emp -> employeeId.equals(emp.getId()))).map(Stream::findFirst)
				.filter(Optional::isPresent).map(Optional::get);
	}

	/**
	 * Gets the list of all {@link Employee}s from the sample dataset.
	 * 
	 * @return the list of {@link Employee}s
	 */
	public static List<Employee> getEmployees() {

		return departmentMap.values().stream().map(dep -> Optional.ofNullable(dep.getEmployees()).orElse(List.of()))
				.flatMap(List::stream).toList();
	}

	/**
	 * Generates the {@link Department}s.
	 * 
	 */
